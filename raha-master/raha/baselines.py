...
class Baselines:
    ...
    def __init__(self):
        ...
        self.DATASET_CONSTRAINTS = {
            ...
            "movies": {
                "functions": [],
                "patterns": [["Id", "^tt[\d]+$", "ONM"], ["First Shown Year", "^[\d]{4}$", "ONM"],
                             ["RatingValue", "^[\d.]*$", "ONM"],
                             ["Duration", "^([\d]+[ ]min)*$", "ONM"]]
            },
        }
    ...
    def run_activeclean(self, dd, sampling_budget=20):
        ...
        while len(labeled_tuples) < sampling_budget:
            ...
            if sum(y_train) == len(y_train):
                ...
                model = sklearn.linear_model.SGDClassifier(loss="log_loss", alpha=1e-6, max_iter=200, fit_intercept=True)
